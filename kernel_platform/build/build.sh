...
if [ "${LTO}" = "none" -o "${LTO}" = "thin" -o "${LTO}" = "full" ]; then
  ...
  # M55 code for SR-QN6887A-01-314 by tangxiaoxing at 20231130 start
  if [ "${ODM_CUSTOM_FACTORY_BUILD}" == "true" ];then
    ${KERNEL_DIR}/scripts/config --file ${OUT_DIR}/.config \
	   -d CONFIG_DEBUG_KMEMLEAK \
	   -d CONFIG_DEBUG_LOCK_ALLOC \
	   -d CONFIG_DEBUG_MUTEXES \
	   -d CONFIG_DEBUG_PREEMPT \
	   -d CONFIG_DEBUG_RT_MUTEXES \
	   -d CONFIG_DEBUG_RWSEMS \
	   -d CONFIG_DEBUG_SPINLOCK \
	   -d CONFIG_I2C_CHARDEV \
	   -d CONFIG_KUNIT \
	   -d CONFIG_LOCK_STAT \
	   -d CONFIG_PM_DEBUG \
	   -d CONFIG_RUNTIME_TESTING_MENU \
	   -d CONFIG_SOFTLOCKUP_DETECTOR \
	   -d CONFIG_USB_MON \
	   -d CONFIG_DEBUG_LOCKDEP \
	   -d CONFIG_KUNIT_DEBUGFS \
	   -d CONFIG_DEBUG_OBJECTS \
	   -d CONFIG_DEBUG_OBJECTS_ENABLE_DEFAULT \
	   -d CONFIG_DEBUG_OBJECTS_FREE \
	   -d CONFIG_DEBUG_OBJECTS_TIMERS \
	   -d CONFIG_DEBUG_OBJECTS_WORK
  fi
  # M55 code for SR-QN6887A-01-314 by tangxiaoxing at 20231130 end
  # M55 code for P240117-06821-314 by tangxiaoxing at 20231130 start
  if [ "${ODM_CUSTOM_FACTORY_BUILD}" != "true" ];then
    ${KERNEL_DIR}/scripts/config --file ${OUT_DIR}/.config \
	   -e CONFIG_CPU_FREQ_DEFAULT_GOV_PERFORMANCE  \
	   -d CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL
  fi
  # M55 code for P240117-06821 by tangxiaoxing at 20231130 end
...
mkdir -p ${OUT_DIR}/arch/${ARCH}/boot/dts/vendor/samsung
set -x
